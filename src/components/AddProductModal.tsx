...
const AddProductModal: React.FC<AddProductModalProps> = ({ isOpen, onClose, onSubmit, isLoading }) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (vinNumbers.every(vin => vin.trim() === '')) newErrors.vinNumber = 'At least one VIN number is required';
    if (formData.dimensions.length <= 0) newErrors.dimensions = 'Length must be greater than 0';
    if (formData.dimensions.width <= 0) newErrors.dimensions = 'Width must be greater than 0';
    if (formData.dimensions.height <= 0) newErrors.dimensions = 'Height must be greater than 0';
    if (formData.dimensions.weight <= 0) newErrors.dimensions = 'Weight must be greater than 0';
