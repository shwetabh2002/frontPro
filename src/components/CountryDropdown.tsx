  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      <button
        ...
        disabled={disabled}
        className={`w-full flex items-center justify-between px-4 py-3 border-2 border-blue-300 border-r-0 rounded-l-lg shadow-sm focus:outline-none focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 text-sm transition-all duration-200 min-w-[160px] ${
          disabled 
            ? 'bg-slate-100 text-slate-400 cursor-not-allowed opacity-75' 
            : 'bg-white text-slate-800 hover:bg-blue-50 hover:border-blue-400'
        }`}
      >
        ...
        <svg
          className={`w-4 h-4 text-blue-500 transition-transform ${isOpen ? 'rotate-180' : ''}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      {isOpen && createPortal(
        <div 
          ...
          className="fixed bg-gradient-to-br from-white to-slate-50 border-2 border-blue-300 rounded-xl shadow-2xl z-[99999] flex flex-col overflow-hidden backdrop-blur-sm"
          style={{
        ...
        >
          ...
          <div className="bg-gradient-to-r from-blue-50 to-slate-100 border-b border-blue-200 p-4 flex-shrink-0">
              ...
              <>
                ...
                {searchTerm === '' && (
                  <>
                    ...
                    {getPopularCountries(filteredCountries).map((country, index) => (
                      <button
                        ...
                        className={`w-full flex items-center px-4 py-3 text-left hover:bg-gradient-to-r hover:from-blue-50 hover:to-slate-50 focus:bg-gradient-to-r focus:from-blue-50 focus:to-slate-50 focus:outline-none transition-all duration-200 ${
                          country.dial_code === value ? 'bg-gradient-to-r from-blue-100 to-slate-100 border-l-4 border-blue-500 shadow-sm' : ''
                        }`}
                      >
                    ...
                    <div className="px-4 py-3 bg-gradient-to-r from-slate-100 to-blue-50 border-b border-blue-200">
                      <div className="text-xs font-bold text-blue-600 uppercase tracking-wide flex items-center">
                        <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
